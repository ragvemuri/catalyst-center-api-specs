...
func QuerysitesenergytaskHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/dna/data/api/v1/energy/sites/query%s", cfg.BaseURL, queryString)
...
func CreateQuerysitesenergytaskTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_dna_data_api_v1_energy_sites_query",
		mcp.WithDescription("Query sites energy for the given task ID"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    QuerysitesenergytaskHandler(cfg),
	}
}
