...
func ReadsitesenergyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/dna/data/api/v1/energy/sites%s", cfg.BaseURL, queryString)
...
func CreateReadsitesenergyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_dna_data_api_v1_energy_sites",
		mcp.WithDescription("Get sites energy"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ReadsitesenergyHandler(cfg),
	}
}
