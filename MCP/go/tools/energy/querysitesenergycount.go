...
func QuerysitesenergycountHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/dna/data/api/v1/energy/sites/query/count", cfg.BaseURL)
...
func CreateQuerysitesenergycountTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_dna_data_api_v1_energy_sites_query_count",
		mcp.WithDescription("Submit request to count sites energy from query"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    QuerysitesenergycountHandler(cfg),
	}
}
